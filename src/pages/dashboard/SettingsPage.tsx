  ...
  return (
    ...
    >
      ...
      >
        ...
        <Box>
          <Typography
            sx={{ fontSize: ".88rem", fontWeight: 700, color: t.text }}
          >
            {title}
          </Typography>
      ...
      <Box sx={{ p: 2.2, display: "flex", flexDirection: "column", gap: 1.8 }}>
        {children}
      </Box>
    </Box>
  );
    ...
    >
      <Box sx={{ minWidth: 0 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 0.6 }}>
          <Typography
            sx={{ fontSize: ".82rem", color: t.text, fontWeight: 500 }}
          >
            {label}
          </Typography>
          {tooltip && (
            <Tooltip title={tooltip}>
              <InfoOutlinedIcon sx={{ fontSize: 13, color: t.textMuted }} />
            </Tooltip>
          )}
          {tag && (
            <Box
            ...
            >
              <Typography
                sx={{ fontSize: ".6rem", color: t.textMuted, fontWeight: 600 }}
              >
                {tag}
              </Typography>
            </Box>
        ...
        <Typography sx={{ fontSize: ".7rem", color: t.textMuted, mt: "1px" }}>
          {description}
        </Typography>
      </Box>
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    const resolved =
      (user as any)?.name || (user?.email ? user.email.split("@")[0] : "User");
    ...
    const loadSettings = async () => {
      try {
        ...
        if (data.alerts) {
          ...
          setEmailSeverityThreshold(
            data.alerts.email_severity_threshold ?? "high",
          );
        }
      } catch {}
    };
      ...
      await apiFetch("/api/settings", {
        ...
        body: JSON.stringify({
          alerts: {
            ...
            email_severity_threshold: emailSeverityThreshold,
          },
        }),
      });
    } catch {
  ...
  const displayRole = (user?.role || "viewer").replace(/^\w/, (c: string) =>
    c.toUpperCase(),
  );
      ...
      <Box sx={{ p: "24px 36px 36px" }}>
        ...
        >
          ...
          <Box
            ...
            sx={{
              ...
              transition: "all .25s",
              "&:hover":
                dirty && !saving ? { boxShadow: `0 0 20px ${ACCENT}40` } : {},
            }}
          >
            <SaveIcon
              sx={{ fontSize: 15, color: dirty ? "#fff" : t.textMuted }}
            />
            <Typography
              sx={{
                fontSize: "0.82rem",
        ...
        >
          <Box
            sx={{
              ...
              height: 48,
              borderRadius: "50%",
              background: `linear-gradient(135deg, ${ACCENT}, #6E1E13)`,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              fontSize: "1.15rem",
          ...
          <Box sx={{ flex: 1, minWidth: 0 }}>
            {editingName ? (
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: 0.8 }}>
                <Typography
                  sx={{ fontSize: "0.98rem", fontWeight: 700, color: t.text }}
                >
                  {displayName}
                </Typography>
                <Box
                  ...
                  sx={{
                    ...
                    cursor: "pointer",
                    color: t.textMuted,
                    "&:hover": { color: ACCENT, background: `${ACCENT}12` },
                  }}
                >
            ...
            <Typography
              sx={{ fontSize: "0.78rem", color: t.textMuted, mt: "2px" }}
            >
              {user?.email || "-"}
            </Typography>
          </Box>
          ...
          >
            <Typography
              sx={{ fontSize: ".7rem", fontWeight: 700, color: ACCENT }}
            >
              {displayRole}
            </Typography>
          </Box>
        ...
        >
          ...
          >
            <Row label="Password" description="Change your account password">
              {showPasswordForm ? (
                ...
                >
                  <Typography
                    sx={{ fontSize: ".76rem", color: t.textSecondary }}
                  >
                    Change
                  </Typography>
                </Box>
          ...
          <Card
            ...
            title="Notifications"
            subtitle="How you're alerted to new incidents"
            accentColor={AMBER}
          >
            <Row
              label="Email alerts"
              description="Get emailed when a new incident is detected"
            >
              <Switch
                checked={emailAlerts}
                onChange={(e) => {
            ...
            <Row
              ...
              description="Only notify above this level"
              tooltip="Applies to email alerts only"
            >
              <FormControl size="small" disabled={!emailAlerts}>
                <Select
                  value={emailSeverityThreshold}
                  onChange={(e) => {
                    setEmailSeverityThreshold(e.target.value);
                    mark();
                  }}
                  sx={{ ...selectSx, opacity: emailAlerts ? 1 : 0.5 }}
                >
        ...
        >
          <Card
            ...
            title="Appearance"
            subtitle="How ONVXP looks on this device"
            accentColor="#7F77DD"
          >
            <Row label="Theme" description="Switch between dark and light mode">
              ...
              >
                {mode === "dark" ? (
                  <LightModeIcon sx={{ fontSize: 14, color: AMBER }} />
                ) : (
                  <DarkModeIcon sx={{ fontSize: 14, color: ACCENT }} />
                )}
                <Typography
          ...
          >
            <Row label="This device" description="Current browser session">
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                >
                  <Typography
                    sx={{ fontSize: ".68rem", fontWeight: 700, color: GREEN }}
                  >
                    Active now
                  </Typography>
                </Box>
        ...
        <Box sx={{ mb: 2.2 }}>
          <Card
            ...
            subtitle="Defaults for how you use ONVXP"
            accentColor={ACCENT}
          >
            <Row
              label="Default landing page"
              description="Where you land right after signing in"
            >
              <FormControl size="small">
                <Select
                  value={prefs.landingPage}
                  onChange={(e) => updatePrefs({ landingPage: e.target.value })}
                  sx={selectSx}
                >
            ...
            <Row
              ...
              description="Pre-selected whenever you open Analytics"
            >
              <FormControl size="small">
                <Select
                  value={prefs.defaultAnalyticsRange}
                  onChange={(e) =>
                    updatePrefs({ defaultAnalyticsRange: e.target.value })
                  }
                  sx={selectSx}
                >
            ...
            <Row
              label="Alert sound"
              description="Play a sound when a new incident comes in"
            >
              <Switch
                checked={prefs.alertSound}
                onChange={(e) => updatePrefs({ alertSound: e.target.checked })}
                size="small"
                sx={{
                  "& .MuiSwitch-switchBase.Mui-checked": { color: ACCENT },
                  "& .MuiSwitch-switchBase.Mui-checked + .MuiSwitch-track": {
            ...
            <Row
              label="Timezone"
              description="Used to display timestamps across the app"
            >
              <FormControl size="small">
                <Select
        ...
        >
          ...
          >
            <Box>
              <Typography
                sx={{ fontSize: "0.86rem", fontWeight: 600, color: "#E74C3C" }}
              >
                Danger Zone
              </Typography>
              <Typography
                sx={{ fontSize: "0.72rem", color: t.textMuted, mt: "2px" }}
              >
