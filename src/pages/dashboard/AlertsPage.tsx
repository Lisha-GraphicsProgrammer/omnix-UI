  ...
  return (
    <Box>
      ...
      >
        <Box>
          <Typography
            sx={{ color: t.text, fontWeight: 700, fontSize: "1.1rem", letterSpacing: "-.3px" }}
          >
            Alert Dashboard
          </Typography>
          <Typography
            sx={{ color: apiError ? "#fca5a5" : t.textMuted, fontSize: ".78rem", mt: 0.2 }}
          >
            {apiError
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          ...
          <Box
            ...
            sx={{
              ...
              transition: "all .2s",
              "&:hover": { transform: "translateY(-1px)", boxShadow: `0 6px 20px ${ACCENT}40` },
            }}
          >
      ...
      <Box sx={{ p: 4 }}>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 2, mb: 4 }}>
          {statCards.map((s, i) => (
            <Box
              key={i}
              sx={{
                p: "20px 24px", borderRadius: "14px",
                background: t.surface, border: `1px solid ${t.border}`,
                display: "flex", alignItems: "center", gap: 2,
                position: "relative", overflow: "hidden",
                transition: "all .25s",
                "&:hover": { transform: "translateY(-2px)", boxShadow: `0 12px 32px ${s.c}18`, borderColor: `${s.c}30` },
                "&::before": { content: '""', position: "absolute", top: 0, left: 0, right: 0, height: "2px", background: `linear-gradient(90deg, transparent, ${s.c}, transparent)` },
              }}
            >
        ...
        >
          ...
          <Box
            sx={{
              px: 3, py: 2.5, borderBottom: `1px solid ${t.border}`,
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              ...
              <Box sx={{ px: 1.5, py: 0.4, borderRadius: "20px", background: `${ACCENT}12`, border: `1px solid ${ACCENT}25` }}>
                <Typography sx={{ color: ACCENT, fontSize: ".65rem", fontWeight: 700 }}>{filtered.length} events</Typography>
              </Box>
            </Box>
              ...
              {["All", "Critical", "High", "Medium"].map((f) => (
                <Box
                  ...
                  sx={{
                    ...
                    border: filter === f ? `1px solid ${ACCENT}35` : `1px solid ${t.border}`,
                    cursor: "pointer", transition: "all .2s",
                  }}
                >
                  <Typography sx={{ color: filter === f ? ACCENT : t.textMuted, fontSize: ".72rem", fontWeight: filter === f ? 700 : 400 }}>
                    {f}
                  </Typography>
                </Box>
              ))}
          ...
          <Box
            sx={{
              display: "grid", gridTemplateColumns: "2fr 1.5fr 1fr 1fr 1fr 0.7fr",
              px: 3, py: 1.5, borderBottom: `1px solid ${t.border}`,
              background: t.surface, flexShrink: 0,
            }}
          >
            {["Camera", "Rule Violated", "Time", "Status", "Severity", "Action"].map((h) => (
              <Typography key={h} sx={{ color: t.textMuted, fontSize: ".65rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".08em" }}>
                {h}
              </Typography>
            ))}
          </Box>
          ...
          >
            {loading ? (
              <Box sx={{ p: 8, textAlign: "center" }}>
                <Box sx={{ width: 32, height: 32, borderRadius: "50%", border: `3px solid ${ACCENT}20`, borderTopColor: ACCENT, animation: "sp 1s linear infinite", "@keyframes sp": { "100%": { transform: "rotate(360deg)" } }, mx: "auto", mb: 2 }} />
                <Typography sx={{ color: t.textMuted, fontSize: ".85rem" }}>Loading incidents...</Typography>
              </Box>
            ) : filtered.length === 0 ? (
              <Box sx={{ p: 8, textAlign: "center" }}>
                <Typography sx={{ color: t.textMuted, fontSize: ".9rem", mb: 0.5 }}>
                ...
                return (
                  ...
                  >
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                      <Box sx={{ width: 8, height: 8, borderRadius: "50%", background: ACCENT, flexShrink: 0, boxShadow: `0 0 6px ${ACCENT}`, animation: "blink 1s infinite", "@keyframes blink": { "0%,100%": { opacity: 1 }, "50%": { opacity: 0.2 } } }} />
                    ...
                    <Box sx={{ display: "inline-flex", alignItems: "center", gap: 0.7, px: 1.2, py: 0.5, borderRadius: "8px", background: `${ACCENT}10`, border: `1px solid ${ACCENT}25`, width: "fit-content" }}>
                      <WarningAmberIcon sx={{ fontSize: 11, color: ACCENT }} />
                      <Typography sx={{ color: ACCENT, fontSize: ".68rem", fontWeight: 600 }}>Active</Typography>
                    </Box>
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                      <Typography sx={{ fontSize: ".78rem", fontWeight: 600, color: ACCENT }}>View</Typography>
          ...
          <Box
            sx={{
              display: "flex", alignItems: "center", justifyContent: "space-between",
              px: 3, py: 1.75, borderTop: `1px solid ${t.border}`,
              flexWrap: "wrap", gap: 1.5, flexShrink: 0,
            }}
          >
            <Typography sx={{ color: t.textMuted, fontSize: ".75rem" }}>
              {total === 0 ? "No violations" : `Showing ${showFrom}–${showTo} of ${total}`}
            </Typography>
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                <Typography sx={{ color: t.textMuted, fontSize: ".72rem" }}>Rows per page</Typography>
                <Select
                  value={pageSize}
                  onChange={(e) => { setPageSize(Number(e.target.value)); setPage(1); }}
                  size="small"
                  sx={{
                    height: 30, minWidth: 68, fontSize: ".75rem",
                    color: t.text, borderRadius: "8px", background: t.surface,
                    ...
                    slotProps: {
                      paper: {
                        sx: {
                          ...
                          "& .MuiMenuItem-root": {
                            fontSize: ".78rem", color: t.textSecondary,
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: 0.75 }}>
                <Box
                  ...
                  sx={{
                    px: 1.4, py: 0.4, borderRadius: "7px",
                    cursor: page > 1 ? "pointer" : "default",
                    fontSize: ".72rem", fontWeight: 600,
                    color: t.textSecondary, opacity: page > 1 ? 1 : 0.35,
                    border: `1px solid ${t.border}`, transition: "all .15s",
                    "&:hover": page > 1 ? { background: t.surfaceHover, borderColor: `${ACCENT}40` } : {},
                  }}
                >
                  ‹ Prev
                </Box>
                <Typography sx={{ color: t.textSecondary, fontSize: ".72rem", fontWeight: 600, minWidth: 54, textAlign: "center" }}>
                  {page} / {totalPages}
                </Typography>
                <Box
                  ...
                  sx={{
                    ...
                    fontSize: ".72rem", fontWeight: 600,
                    color: t.textSecondary, opacity: page < totalPages ? 1 : 0.35,
                    border: `1px solid ${t.border}`, transition: "all .15s",
                    "&:hover": page < totalPages ? { background: t.surfaceHover, borderColor: `${ACCENT}40` } : {},
                  }}
                >
                  Next ›
                </Box>
