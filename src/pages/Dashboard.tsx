  ...
  return (
    <Box sx={{ px: 4, py: 2.5, borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(13,13,16,0.8)", backdropFilter: "blur(12px)", position: "sticky", top: 0, zIndex: 50 }}>
      <Box>
        ...
        <Typography sx={{ color: "rgba(255,255,255,0.25)", fontSize: ".78rem", mt: 0.2 }}>{subtitle}</Typography>
      </Box>
...
function MetricTile({ icon, value, label, color, pulse = false }: { icon: React.ReactNode; value: string; label: string; color: string; pulse?: boolean }) {
  return (
    <Box sx={{ flex: 1, minWidth: 0, background: CARD_BG, border: CARD_BORDER, borderRadius: "14px", p: "20px 24px", display: "flex", alignItems: "center", gap: 2, position: "relative", overflow: "hidden",
      ...
      <Box>
        ...
        <Typography sx={{ fontSize: "0.72rem", color: "rgba(255,255,255,0.4)", mt: "2px", letterSpacing: "0.04em", textTransform: "uppercase" }}>{label}</Typography>
      </Box>
    </Box>
  );
}
...
function SectionCard({ icon, title, subtitle, accentColor, children }: { icon: React.ReactNode; title: string; subtitle: string; accentColor: string; children: React.ReactNode }) {
  return (
    <Box sx={{ background: CARD_BG, border: CARD_BORDER, borderRadius: "16px", overflow: "hidden", height: "100%" }}>
      <Box sx={{ px: 3, py: "18px", background: `linear-gradient(135deg, ${accentColor}12 0%, transparent 60%)`, borderBottom: `1px solid ${accentColor}20`, display: "flex", alignItems: "center", gap: "14px" }}>
        ...
        <Box>
          <Typography sx={{ fontWeight: 600, fontSize: "0.95rem", color: "#fff" }}>{title}</Typography>
          <Typography sx={{ fontSize: "0.73rem", color: "rgba(255,255,255,0.4)", mt: "1px" }}>{subtitle}</Typography>
        </Box>
      </Box>
...
function SettingRow({ label, description, tag, tagColor = CYAN, children, tooltip }: { label: string; description: string; tag?: string; tagColor?: string; children: React.ReactNode; tooltip?: string }) {
  return (
    <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", py: "14px", gap: 2, "&:not(:last-child)": { borderBottom: "1px solid rgba(255,255,255,0.05)" } }}>
      <Box sx={{ flex: 1, minWidth: 0 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <Typography sx={{ fontSize: "0.875rem", fontWeight: 500, color: "rgba(255,255,255,0.88)" }}>{label}</Typography>
          {tooltip && (
            <Tooltip title={tooltip} arrow placement="top">
              <InfoOutlinedIcon sx={{ fontSize: 14, color: "rgba(255,255,255,0.25)", cursor: "help" }} />
            </Tooltip>
          )}
        </Box>
        <Typography sx={{ fontSize: "0.73rem", color: "rgba(255,255,255,0.35)", mt: "2px" }}>{description}</Typography>
      </Box>
      <Box sx={{ display: "flex", alignItems: "center", gap: "10px", flexShrink: 0 }}>
        {tag && (
          <Chip label={tag} size="small" sx={{ height: 20, fontSize: "0.65rem", fontWeight: 600, letterSpacing: "0.04em", background: `${tagColor}18`, color: tagColor, border: `1px solid ${tagColor}30`, borderRadius: "5px" }} />
        )}
        {children}
      </Box>
    </Box>
...
function ValuePill({ value, highlight = false }: { value: string; highlight?: boolean }) {
  return (
    <Box sx={{ px: "14px", py: "5px", borderRadius: "8px", background: highlight ? `${CYAN}15` : "rgba(255,255,255,0.06)", border: `1px solid ${highlight ? CYAN + "35" : "rgba(255,255,255,0.1)"}`, color: highlight ? CYAN : "rgba(255,255,255,0.75)", fontSize: "0.82rem", fontWeight: 600, fontFamily: '"JetBrains Mono", "Fira Code", monospace', whiteSpace: "nowrap" }}>
      {value}
    </Box>
  );
}

// ─── CamerasPage (unchanged) ──────────────────────────────────────────────────
function CamerasPage() {
    ...
    const fetchCam1 = async () => {
      try {
        ...
        if (data && data.length > 0) {
          const latest = data[0];
          setCam1Data({ thumbnail: latest.screenshot_url || null, lastDetection: latest.timestamp || null });
        }
  ...
  return (
    <Box>
      <Box sx={{ px: 4, py: 2.5, borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(13,13,16,0.9)", backdropFilter: "blur(12px)", position: "sticky", top: 0, zIndex: 50 }}>
        <Box>
          <Typography sx={{ color: "#fff", fontWeight: 700, fontSize: "1.1rem", letterSpacing: "-.3px" }}>Camera Management</Typography>
          <Typography sx={{ color: "rgba(255,255,255,0.25)", fontSize: ".78rem", mt: .2 }}>8 cameras configured · 7 online · Site A</Typography>
        </Box>
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          ...
          <Box sx={{ px: 2.5, py: 1, borderRadius: "10px", background: "linear-gradient(135deg, #6366f1, #7c3aed)", border: "1px solid rgba(99,102,241,0.3)", cursor: "pointer", boxShadow: "0 4px 14px rgba(99,102,241,0.25)", transition: "all .2s", "&:hover": { transform: "translateY(-1px)", boxShadow: "0 6px 20px rgba(99,102,241,0.35)" } }}>
      ...
      <Box sx={{ p: 4 }}>
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 2, mb: 4 }}>
          {[
            ...
            { val: "7", label: "Online",         sub: "Streaming live",      c: "#00E676", icon: <CheckCircleIcon sx={{ fontSize: 20 }} /> },
            { val: "1", label: "Offline",         sub: "Needs attention",    c: "#FF4444", icon: <WifiIcon sx={{ fontSize: 20 }} /> },
            { val: "4", label: "Active Alerts",   sub: "Violations detected", c: "#FFB300", icon: <WarningAmberIcon sx={{ fontSize: 20 }} /> },
          ].map((s, i) => (
            <Box key={i} sx={{ p: "20px 24px", borderRadius: "14px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", gap: 2, position: "relative", overflow: "hidden", transition: "all .25s", "&:hover": { transform: "translateY(-2px)", boxShadow: `0 12px 32px ${s.c}18`, borderColor: `${s.c}30` }, "&::before": { content: '""', position: "absolute", top: 0, left: 0, right: 0, height: "2px", background: `linear-gradient(90deg, transparent, ${s.c}, transparent)` } }}>
              ...
              <Box>
                ...
                <Typography sx={{ color: "#fff", fontSize: ".82rem", fontWeight: 600, mt: ".2rem" }}>{s.label}</Typography>
                <Typography sx={{ color: "rgba(255,255,255,0.28)", fontSize: ".68rem", mt: ".1rem" }}>{s.sub}</Typography>
              </Box>
            </Box>
          ))}
        </Box>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 2.5 }}>
          {mockCameras.map((cam) => (
            <Box key={cam.id} sx={{ borderRadius: "20px", overflow: "hidden", background: "rgba(255,255,255,0.02)", border: `1px solid ${cam.status === "offline" ? "rgba(239,68,68,0.2)" : "rgba(255,255,255,0.07)"}`, transition: "all .25s", boxShadow: "0 4px 16px rgba(0,0,0,0.3)", "&:hover": { border: `1px solid ${cam.status === "offline" ? "rgba(239,68,68,0.35)" : "rgba(99,102,241,0.3)"}`, transform: "translateY(-3px)", boxShadow: "0 12px 32px rgba(0,0,0,0.4)" } }}>
                ...
                {cam.status === "online" ? (
                  <>
                    {cam.id === 1 && cam1Data.thumbnail ? (
                      ...
                      <>
                        <Box sx={{ position: "absolute", inset: 0, backgroundImage: "linear-gradient(rgba(99,102,241,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(99,102,241,0.03) 1px, transparent 1px)", backgroundSize: "24px 24px" }} />
                    ...
                    <Box sx={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "40%", background: "linear-gradient(to top, rgba(0,0,0,0.6), transparent)", zIndex: 1 }} />
                  </>
                ) : (
              ...
              <Box sx={{ p: "14px 16px" }}>
                <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: .6 }}>
                  <Typography sx={{ color: "#fff", fontSize: ".8rem", fontWeight: 600, letterSpacing: "-.2px" }} noWrap>{cam.name}</Typography>
                  <WifiIcon sx={{ fontSize: 13, color: cam.status === "online" ? "#6ee7b7" : "#fca5a5", flexShrink: 0, ml: 1 }} />
                </Box>
                <Typography sx={{ color: "rgba(255,255,255,0.22)", fontSize: ".7rem", mb: .8 }}>{cam.location}</Typography>
                {cam.id === 1 && cam1Data.lastDetection && (
                  <Box sx={{ display: "inline-flex", alignItems: "center", gap: .6, mb: 1, px: 1, py: .35, borderRadius: "6px", background: "rgba(239,68,68,0.07)", border: "1px solid rgba(239,68,68,0.15)" }}>
                    ...
                    <Typography sx={{ color: "rgba(239,68,68,0.7)", fontSize: ".62rem" }}>
                      Last: {new Date(cam1Data.lastDetection).toLocaleTimeString()}
                    </Typography>
                  </Box>
                )}
...
function AlertsPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  const statCards = [
    ...
    { val: "8", label: "Active Cameras", sub: "100% online", c: "#818cf8", icon: <CameraAltIcon sx={{ fontSize: 20 }} /> },
    { val: stats.zones_affected.length.toString(), label: "Zones Affected", sub: "With active violations", c: "#00E676", icon: <CheckCircleIcon sx={{ fontSize: 20 }} /> },
    { val: stats.unique_persons.toString(), label: "Unique Persons", sub: "ByteTrack deduplicated", c: "#FFB300", icon: <PersonIcon sx={{ fontSize: 20 }} /> },
  ];
  ...
  return (
    <Box>
      <Box sx={{ px: 4, py: 2.5, borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(13,13,16,0.9)", backdropFilter: "blur(12px)", position: "sticky", top: 0, zIndex: 50 }}>
        <Box>
          <Typography sx={{ color: "#fff", fontWeight: 700, fontSize: "1.1rem", letterSpacing: "-.3px" }}>Active Alerts</Typography>
          <Typography sx={{ color: apiError ? "#fca5a5" : "rgba(255,255,255,0.25)", fontSize: ".78rem", mt: .2 }}>
            {apiError ? "⚠ API offline — showing cached state" : `Real-time violation monitoring — ${alerts.length} events`}
          </Typography>
        </Box>
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <Box sx={{ width: 7, height: 7, borderRadius: "50%", background: apiError ? "#ef4444" : "#22c55e", boxShadow: `0 0 8px ${apiError ? "rgba(239,68,68,0.6)" : "rgba(34,197,94,0.6)"}`, animation: "pg 2s infinite", "@keyframes pg": { "0%,100%": { opacity: 1 }, "50%": { opacity: .3 } } }} />
            <Typography sx={{ color: "rgba(255,255,255,0.25)", fontSize: ".75rem" }}>{apiError ? "Offline" : "Live"}</Typography>
          </Box>
          <Box onClick={() => navigate("/rules")} sx={{ px: 2.5, py: 1, borderRadius: "10px", background: "linear-gradient(135deg, #6366f1, #7c3aed)", border: "1px solid rgba(99,102,241,0.3)", cursor: "pointer", boxShadow: "0 4px 14px rgba(99,102,241,0.25)", transition: "all .2s", "&:hover": { transform: "translateY(-1px)", boxShadow: "0 6px 20px rgba(99,102,241,0.35)" } }}>
      ...
      <Box sx={{ p: 4 }}>
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 2, mb: 4 }}>
          {statCards.map((s, i) => (
            <Box key={i} sx={{ p: "20px 24px", borderRadius: "14px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", gap: 2, position: "relative", overflow: "hidden", transition: "all .25s", cursor: "default", "&:hover": { transform: "translateY(-2px)", boxShadow: `0 12px 32px ${s.c}18`, borderColor: `${s.c}30` }, "&::before": { content: '""', position: "absolute", top: 0, left: 0, right: 0, height: "2px", background: `linear-gradient(90deg, transparent, ${s.c}, transparent)` } }}>
              <Box sx={{ width: 44, height: 44, borderRadius: "12px", background: `${s.c}18`, border: `1px solid ${s.c}40`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <Box sx={{ color: s.c, display: "flex" }}>{s.icon}</Box>
              </Box>
              <Box>
        ...
        <Box sx={{ borderRadius: "20px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", overflow: "hidden", boxShadow: "0 8px 32px rgba(0,0,0,0.3)" }}>
          <Box sx={{ px: 3, py: 2.5, borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "space-between", background: "linear-gradient(135deg, rgba(99,102,241,0.04), rgba(139,92,246,0.02))" }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              <Typography sx={{ color: "#fff", fontWeight: 700, fontSize: ".92rem", letterSpacing: "-.2px" }}>Violation Log</Typography>
            ...
            <Box sx={{ display: "flex", gap: 1 }}>
              {["All", "Critical", "High", "Medium"].map((f) => (
                <Box key={f} onClick={() => setFilter(f)} sx={{ px: 1.5, py: .5, borderRadius: "8px", background: filter === f ? "rgba(99,102,241,0.15)" : "transparent", border: filter === f ? "1px solid rgba(99,102,241,0.3)" : "1px solid rgba(255,255,255,0.06)", cursor: "pointer", transition: "all .2s", "&:hover": { background: "rgba(99,102,241,0.08)" } }}>
                  <Typography sx={{ color: filter === f ? "#818cf8" : "rgba(255,255,255,0.3)", fontSize: ".72rem", fontWeight: filter === f ? 700 : 400 }}>{f}</Typography>
                </Box>
              ))}
            </Box>
          </Box>
          ...
          {loading ? (
            <Box sx={{ p: 8, textAlign: "center" }}>
              ...
              <Typography sx={{ color: "rgba(255,255,255,0.25)", fontSize: ".85rem" }}>Loading incidents...</Typography>
            </Box>
          ) : filtered.length === 0 ? (
            <Box sx={{ p: 8, textAlign: "center" }}>
              <Typography sx={{ color: "rgba(255,255,255,0.15)", fontSize: "2rem", mb: 1 }}>🔍</Typography>
              <Typography sx={{ color: "rgba(255,255,255,0.3)", fontSize: ".9rem", mb: .5 }}>
                {apiError ? "Cannot reach API at localhost:8000" : "No violations match this filter"}
              </Typography>
              <Typography sx={{ color: "rgba(255,255,255,0.15)", fontSize: ".78rem" }}>
                {apiError ? "Start the uvicorn server to see live data" : "Try selecting a different filter"}
              </Typography>
            </Box>
          ) : (
            <Box>
              <Box sx={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 1fr 1fr 1fr 0.7fr", px: 3, py: 1.5, borderBottom: "1px solid rgba(255,255,255,0.04)", background: "rgba(255,255,255,0.01)" }}>
                {["Camera", "Rule Violated", "Time", "Status", "Severity", "Action"].map((h) => (
                  <Typography key={h} sx={{ color: "rgba(255,255,255,0.18)", fontSize: ".65rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".08em" }}>{h}</Typography>
                ))}
              </Box>
              {filtered.map((alert, idx) => {
                const sev = severityConfig[alert.severity];
                return (
                  <Box key={alert.id} onClick={() => navigate(`/alert/${alert.id}`)} sx={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 1fr 1fr 1fr 0.7fr", px: 3, py: 2.5, alignItems: "center", borderBottom: idx < filtered.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none", cursor: "pointer", transition: "all .15s", "&:hover": { background: "rgba(99,102,241,0.05)" } }}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                      <Box sx={{ width: 8, height: 8, borderRadius: "50%", background: "#ef4444", flexShrink: 0, boxShadow: "0 0 6px #ef4444", animation: "blink 1s infinite", "@keyframes blink": { "0%,100%": { opacity: 1 }, "50%": { opacity: .2 } } }} />
                      <Typography sx={{ color: "#fff", fontSize: ".84rem", fontWeight: 500 }}>{alert.camera}</Typography>
                    </Box>
                    <Typography sx={{ color: "rgba(255,255,255,0.55)", fontSize: ".82rem" }}>{alert.rule}</Typography>
                    <Typography sx={{ color: "rgba(255,255,255,0.3)", fontSize: ".8rem", fontFamily: "monospace" }}>{alert.time}</Typography>
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", gap: .5, color: "#6366f1" }}>
...
function SettingsPage() {
  ...
  const handleReset = () => {
    setCooldown(150); setConfidence(0.50); setBytetrackBuffer(30);
    setDedup(true); setAlertChannel("dashboard"); setEmailAlerts(false);
    setFrameSampling("every"); setModelPrecision("balanced");
    setSiteName("Site A — Construction"); setApiEndpoint("http://localhost:8000");
    setLlmModel("claude-haiku");
    setDirty(false);
  };

  const selectSx = {
    minWidth: 160, fontSize: "0.82rem",
  ...
  return (
    <Box sx={{ p: "32px 36px", minHeight: "100vh" }}>
      ...
      <Box sx={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", mb: 4 }}>
        <Box>
          <Typography sx={{ fontSize: "1.5rem", fontWeight: 700, color: "#fff", letterSpacing: "-0.02em" }}>Settings</Typography>
          <Typography sx={{ fontSize: "0.8rem", color: "rgba(255,255,255,0.35)", mt: "4px" }}>Platform configuration — OMNIX POC v0.1</Typography>
        </Box>
        <Box sx={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <Tooltip title="Reset to defaults">
            <IconButton onClick={handleReset} size="small" sx={{ border: "1px solid rgba(255,255,255,0.1)", borderRadius: "10px", color: "rgba(255,255,255,0.4)", "&:hover": { borderColor: "rgba(255,255,255,0.25)", color: "#fff" } }}>
              <RestoreIcon fontSize="small" />
            </IconButton>
          </Tooltip>
          <Box
            onClick={dirty && !saving ? handleSave : undefined}
            sx={{ display: "flex", alignItems: "center", gap: "8px", px: "20px", py: "10px", borderRadius: "10px", background: dirty ? `linear-gradient(135deg, ${PURPLE}, #5B21B6)` : "rgba(255,255,255,0.06)", border: `1px solid ${dirty ? PURPLE + "80" : "rgba(255,255,255,0.1)"}`, cursor: dirty && !saving ? "pointer" : "default", opacity: saving ? 0.7 : 1, transition: "all .25s", "&:hover": dirty && !saving ? { background: "linear-gradient(135deg, #8B5CF6, #7C3AED)", boxShadow: `0 0 20px ${PURPLE}50` } : {} }}
          >
            <SaveIcon sx={{ fontSize: 16, color: dirty ? "#fff" : "rgba(255,255,255,0.3)" }} />
            <Typography sx={{ fontSize: "0.82rem", fontWeight: 600, color: dirty ? "#fff" : "rgba(255,255,255,0.3)" }}>
      ...
      {saving && (
        <LinearProgress sx={{ mb: 3, borderRadius: 2, height: 2, background: "rgba(255,255,255,0.05)", "& .MuiLinearProgress-bar": { background: `linear-gradient(90deg, ${PURPLE}, ${CYAN})` } }} />
      )}
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 3 }}>
        ...
        <SectionCard icon={<TuneIcon fontSize="small" />} title="Detection Engine" subtitle="Configure core CV pipeline parameters" accentColor={CYAN}>
          <SettingRow label="Alert Cooldown" description="Minimum frames between alerts for same person ID" tag="ByteTrack" tooltip="Prevents alert flooding — one person triggers one alert per N frames">
            <Box sx={{ display: "flex", alignItems: "center", gap: 2, width: 210 }}>
              <Slider value={cooldown} min={30} max={500} step={10} onChange={(_, v) => { setCooldown(v as number); mark(); }}
                sx={{ color: CYAN, flex: 1, "& .MuiSlider-thumb": { width: 14, height: 14 }, "& .MuiSlider-rail": { opacity: 0.2 } }} />
          ...
          <SettingRow label="Detection Confidence" description="Minimum YOLO confidence score (0–1)" tag="YOLOv8" tooltip="Lower = more detections but more false positives">
            <Box sx={{ display: "flex", alignItems: "center", gap: 2, width: 210 }}>
              <Slider value={confidence} min={0.1} max={0.95} step={0.01} onChange={(_, v) => { setConfidence(v as number); mark(); }}
                sx={{ color: PURPLE, flex: 1, "& .MuiSlider-thumb": { width: 14, height: 14 }, "& .MuiSlider-rail": { opacity: 0.2 } }} />
              <ValuePill value={confidence.toFixed(2)} highlight />
            </Box>
          </SettingRow>
          <SettingRow label="ByteTrack Buffer" description="Frames to retain lost track IDs" tag="Tracking" tooltip="How long to keep a person's ID alive after they leave frame">
            <Box sx={{ display: "flex", alignItems: "center", gap: 2, width: 210 }}>
              <Slider value={bytetrackBuffer} min={5} max={120} step={5} onChange={(_, v) => { setBytetrackBuffer(v as number); mark(); }}
                sx={{ color: AMBER, flex: 1, "& .MuiSlider-thumb": { width: 14, height: 14 }, "& .MuiSlider-rail": { opacity: 0.2 } }} />
        ...
        <SectionCard icon={<NotificationsActiveIcon fontSize="small" />} title="Alert System" subtitle="Configure how and where alerts are dispatched" accentColor={AMBER}>
          ...
          <SettingRow label="Alert Deduplication" description="Prevent duplicate alerts for the same event" tag="ByteTrack" tooltip="Uses ByteTrack IDs to suppress repeated alerts per individual">
            <Switch checked={dedup} onChange={(e) => { setDedup(e.target.checked); mark(); }} size="small"
          ...
          <SettingRow label="Email Notifications" description="Send alert emails to registered admin address" tag="Optional" tagColor="rgba(255,255,255,0.3)">
            <Switch checked={emailAlerts} onChange={(e) => { setEmailAlerts(e.target.checked); mark(); }} size="small"
              sx={{ "& .MuiSwitch-switchBase.Mui-checked": { color: CYAN }, "& .MuiSwitch-switchBase.Mui-checked + .MuiSwitch-track": { background: CYAN } }} />
          </SettingRow>
        </SectionCard>
        ...
        <SectionCard icon={<SettingsIcon fontSize="small" />} title="Platform" subtitle="General platform settings" accentColor="rgba(255,255,255,0.45)">
          ...
          <SettingRow label="Site Name" description="Current monitoring site identifier" tag="Active" tagColor={GREEN}>
            <TextField value={siteName} onChange={(e) => { setSiteName(e.target.value); mark(); }} size="small"
              sx={{ width: 220, "& .MuiOutlinedInput-root": { fontSize: "0.82rem", background: "rgba(255,255,255,0.04)", borderRadius: "8px", color: "rgba(255,255,255,0.8)", "& fieldset": { borderColor: "rgba(255,255,255,0.1)" }, "&:hover fieldset": { borderColor: "rgba(255,255,255,0.2)" }, "&.Mui-focused fieldset": { borderColor: CYAN } } }} />
          </SettingRow>
          <SettingRow label="API Endpoint" description="Backend FastAPI server address" tag="Local" tagColor="rgba(255,255,255,0.35)">
            <TextField value={apiEndpoint} onChange={(e) => { setApiEndpoint(e.target.value); mark(); }} size="small"
              sx={{ width: 220, "& .MuiOutlinedInput-root": { fontSize: "0.78rem", fontFamily: "monospace", background: "rgba(255,255,255,0.04)", borderRadius: "8px", color: CYAN, "& fieldset": { borderColor: "rgba(255,255,255,0.1)" }, "&:hover fieldset": { borderColor: "rgba(255,255,255,0.2)" }, "&.Mui-focused fieldset": { borderColor: CYAN } } }} />
          </SettingRow>
        </SectionCard>
      ...
      <Box sx={{ mt: 3, p: "20px 24px", borderRadius: "14px", border: "1px solid rgba(255,68,68,0.2)", background: "rgba(255,68,68,0.03)" }}>
        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <Box>
            ...
            <Typography sx={{ fontSize: "0.73rem", color: "rgba(255,255,255,0.3)", mt: "2px" }}>
              Clear pipeline data, reset stored tracks, and flush the alert queue
            </Typography>
          </Box>
          <Box sx={{ display: "flex", gap: "10px" }}>
            {["Flush Alert Queue", "Reset Track IDs"].map((label) => (
              <Box key={label} sx={{ px: "16px", py: "7px", borderRadius: "8px", cursor: "pointer", border: "1px solid rgba(255,68,68,0.3)", color: "#FF6B6B", fontSize: "0.8rem", fontWeight: 500, transition: "all .2s", "&:hover": { background: "rgba(255,68,68,0.1)", borderColor: "rgba(255,68,68,0.5)" } }}>
                {label}
              </Box>
            ))}
          </Box>
      ...
      <Snackbar open={saved} autoHideDuration={3000} onClose={() => setSaved(false)} anchorOrigin={{ vertical: "bottom", horizontal: "right" }}>
        <Alert severity="success" onClose={() => setSaved(false)} sx={{ background: "rgba(0,230,118,0.12)", border: "1px solid rgba(0,230,118,0.3)", color: GREEN, "& .MuiAlert-icon": { color: GREEN } }}>
      ...
      <Box sx={{ width: DRAWER, flexShrink: 0, display: "flex", flexDirection: "column", background: "#0d0d10", borderRight: "1px solid rgba(255,255,255,0.05)", position: "fixed", top: 0, left: 0, bottom: 0, zIndex: 100 }}>
        ...
        <Box sx={{ flex: 1, py: 2 }}>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Box key={item.text} onClick={() => { setSelected(item.text); if (item.text === "Rules") navigate("/rules"); }}
                sx={{ display: "flex", alignItems: "center", gap: 1.5, px: 3, py: 1.4, mx: 1.5, mb: 0.5, borderRadius: "10px", cursor: "pointer", position: "relative", background: isSelected ? "rgba(99,102,241,0.12)" : "transparent", border: isSelected ? "1px solid rgba(99,102,241,0.2)" : "1px solid transparent", transition: "all .2s", "&:hover": { background: isSelected ? "rgba(99,102,241,0.12)" : "rgba(255,255,255,0.04)" } }}>
        ...
        <Box sx={{ p: 2, borderTop: "1px solid rgba(255,255,255,0.05)" }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1, p: "10px 12px", borderRadius: "10px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", mb: 1.5 }}>
            ...
            <Box sx={{ flex: 1, minWidth: 0 }}>
              ...
              <Typography sx={{ color: "rgba(255,255,255,0.25)", fontSize: ".65rem", mt: 0.2 }} noWrap>admin@omnix.ai</Typography>
            </Box>
          </Box>
          <Box onClick={() => { localStorage.removeItem("omnix_auth"); navigate("/login"); }} sx={{ display: "flex", alignItems: "center", gap: 1, px: 1.5, py: 1, borderRadius: "8px", cursor: "pointer", "&:hover": { background: "rgba(255,255,255,0.04)" }, transition: "all .2s" }}>
