...
const DRAFT_KEY        = "omnix_rule_draft";       // ← persist draft
const CHAT_HISTORY_KEY = "omnix_chat_history";      // ← persist chat

// ─── Types ────────────────────────────────────────────────────────────────────
interface RuleHistoryItem {
  id: number; instruction: string; status: "pending" | "active";
  time: string; pipeline: string; alerts: number; config?: any;
}
...
function buildSummary(config: any): string {
  ...
  return `Got it! I'll watch the **${zoneName}** and ${action}. Severity will be **${severity}**, and I'll wait **${cooldown} seconds** between repeat alerts for the same person. Is that right?`;
}
...
// ─── Skeleton loader — matches real confirmation card shape ───────────────────
      ...
      <Box sx={{ flex: 1, borderRadius: "16px 16px 16px 4px", overflow: "hidden", border: `1px solid ${CYAN}25`, background: `${CYAN}04` }}>
        ...
        <Box sx={{ p: "16px 20px" }}>
          ...
          <Box sx={{ display: "flex", gap: 1.5, mb: 1.5 }}>
            <Box sx={{ flex: 1, height: 40, borderRadius: "10px", background: `${GREEN}25`, ...pulse(0.3) }} />
            <Box sx={{ width: 155, height: 40, borderRadius: "10px", background: skBg, ...pulse(0.4) }} />
          </Box>
          {/* Tech details toggle */}
          <Box sx={{ height: 10, width: 130, borderRadius: 5, background: skBg, ...pulse(0.5) }} />
        </Box>
      </Box>
    </Box>
  );
...
export default function Rules() {
  ...
  // Persist draft whenever instruction changes
  useEffect(() => {
    try { localStorage.setItem(DRAFT_KEY, instruction); } catch {}
  }, [instruction]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const assistantMsg: ChatMessage = {
        ...
        config: data.config, instruction: userMsg,
        time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
      };
  ...
  const handleApply = async (config: any, instruction: string) => {
    ...
    try {
      ...
      const newRule: RuleHistoryItem = {
        id: Date.now(), instruction, status: "active",
        time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
  ...
  const handleClarify = (originalInstruction: string) => {
    ...
    <Box sx={{ height: "100vh", display: "flex", background: t.bg, fontFamily: '"Inter", system-ui, sans-serif', overflow: "hidden" }}>
      ...
      <Box sx={{ width: drawerWidth, flexShrink: 0, display: "flex", flexDirection: "column", background: t.sidebarBg, borderRight: `1px solid ${t.border}`, position: "fixed", top: 0, left: 0, bottom: 0, zIndex: 100, transition: "width .25s cubic-bezier(.4,0,.2,1)", overflow: "hidden" }}>
        ...
        <Box sx={{ flex: 1, py: 2, overflowX: "hidden" }}>
          {sidebarOpen && <Typography sx={{ color: t.textMuted, fontSize: ".6rem", fontWeight: 600, letterSpacing: ".1em", textTransform: "uppercase", px: 3, mb: 1, opacity: 0.6 }}>Navigation</Typography>}
          {navItems.map(item => {
            const isSel = item.text === "Rules";
            return (
              <Tooltip key={item.text} title={!sidebarOpen ? item.text : ""} placement="right">
                <Box onClick={() => { if (item.text !== "Rules") navigate(item.path); }}
            ...
            <Box sx={{ p: "40px 48px" }}>
              ...
              <Box sx={{ mb: 4 }}>
                ...
                <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1.5 }}>
                  {suggestions.map((s, i) => (
                    <Box key={i} onClick={() => { setInstruction(s.text); setGeneratedConfig(null); }}
                      sx={{ display: "flex", alignItems: "flex-start", gap: 1.5, p: "14px 16px", borderRadius: "14px", background: t.surface, border: `1px solid ${t.border}`, cursor: "pointer", transition: "all .2s", "&:hover": { background: `${s.iconColor}08`, borderColor: `${s.iconColor}30`, transform: "translateY(-2px)" } }}>
                ...
                <Box sx={{ mb: 3, display: "flex", flexDirection: "column", gap: 2 }}>
                  {chatHistory.map((msg) => (
                    <Box key={msg.id}>
                      ...
                      {/* Assistant message — confirmation card */}
                      {(msg.role === "assistant" || msg.role === "applied") && msg.config && (
                        <Box sx={{ display: "flex", gap: 1.5 }}>
                          <Box sx={{ width: 28, height: 28, borderRadius: "50%", background: `linear-gradient(135deg, ${CYAN}40, ${PURPLE}40)`, border: `1px solid ${CYAN}40`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, mt: 0.5 }}>
                            <SmartToyIcon sx={{ fontSize: 15, color: CYAN }} />
                          </Box>
                          <Box sx={{ flex: 1, maxWidth: "90%" }}>
                            {msg.role === "applied" ? (
                              /* Applied state — compact confirmation */
                              <Box sx={{ p: "12px 16px", borderRadius: "16px 16px 16px 4px", background: `${GREEN}10`, border: `1px solid ${GREEN}25` }}>
                                <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 0.5 }}>
                                  <CheckCircleIcon sx={{ fontSize: 14, color: GREEN }} />
                                  <Typography sx={{ color: GREEN, fontSize: ".75rem", fontWeight: 700 }}>Rule applied & pipeline started</Typography>
                                </Box>
                                <Typography sx={{ color: t.textSecondary, fontSize: ".8rem", lineHeight: 1.5 }}>{msg.instruction}</Typography>
                                <Typography sx={{ color: t.textMuted, fontSize: ".62rem", mt: 0.5 }}>{msg.time}</Typography>
                              </Box>
                            ) : (
                              ...
                              <Box sx={{ borderRadius: "16px 16px 16px 4px", overflow: "hidden", border: `1px solid ${CYAN}30`, background: `${CYAN}05` }}>
                                ...
                                <Box sx={{ p: "16px 20px" }}>
                                  ...
                                  <Box sx={{ display: "flex", gap: 1.5, mb: 1.5, flexWrap: "wrap" }}>
                                    <Box onClick={() => handleApply(msg.config, msg.instruction || "")}
                                      sx={{ flex: 1, minWidth: 160, display: "flex", alignItems: "center", justifyContent: "center", gap: 1, px: "14px", py: "9px", borderRadius: "10px", background: `linear-gradient(135deg, ${GREEN}, #00B248)`, cursor: applying ? "default" : "pointer", boxShadow: `0 4px 14px ${GREEN}30`, opacity: applying ? 0.7 : 1, transition: "all .2s", "&:hover": !applying ? { transform: "translateY(-1px)" } : {} }}>
                                      {applying ? <Box sx={{ width: 12, height: 12, borderRadius: "50%", border: "2px solid rgba(255,255,255,0.2)", borderTopColor: "#fff", animation: "sp 1s linear infinite", "@keyframes sp": { "100%": { transform: "rotate(360deg)" } } }} />
                                        : <CheckCircleIcon sx={{ fontSize: 14, color: "#fff" }} />}
                                      <Typography sx={{ fontSize: ".8rem", fontWeight: 700, color: "#fff" }}>{applying ? "Applying..." : "Yes, apply this rule"}</Typography>
                                    </Box>
                                    <Box onClick={() => handleClarify(msg.instruction || "")}
                                      sx={{ display: "flex", alignItems: "center", gap: 0.8, px: "14px", py: "9px", borderRadius: "10px", background: t.surface, border: `1px solid ${AMBER}40`, cursor: "pointer", transition: "all .2s", "&:hover": { background: `${AMBER}08`, borderColor: `${AMBER}70` } }}>
                                      <EditIcon sx={{ fontSize: 13, color: AMBER }} />
                                      <Typography sx={{ fontSize: ".8rem", fontWeight: 600, color: AMBER }}>No, let me clarify</Typography>
                                    </Box>
                                  </Box>
                                  {/* Tech details */}
                                  <Box onClick={() => setShowTechDetails(v => !v)}
                                    sx={{ display: "flex", alignItems: "center", gap: 0.8, cursor: "pointer", width: "fit-content", py: 0.5, px: 1, borderRadius: "6px", "&:hover": { background: t.surfaceHover } }}>
                                    {showTechDetails ? <ExpandLessIcon sx={{ fontSize: 14, color: t.textMuted }} /> : <ExpandMoreIcon sx={{ fontSize: 14, color: t.textMuted }} />}
                                    <Typography sx={{ color: t.textMuted, fontSize: ".72rem" }}>{showTechDetails ? "Hide" : "Show"} technical details</Typography>
                                  </Box>
              ...
              {error && (
                <Box sx={{ mb: 2, p: "14px 18px", borderRadius: "12px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.3)", display: "flex", alignItems: "flex-start", gap: 1.5 }}>
                  ...
                  <Box sx={{ flex: 1 }}>
                    <Typography sx={{ color: "#fca5a5", fontSize: ".85rem", fontWeight: 600, mb: 0.5 }}>Generation failed</Typography>
              ...
              <Box sx={{ borderRadius: "16px", background: t.surface, border: `1px solid ${instruction ? `${PURPLE}40` : t.border}`, overflow: "hidden", transition: "all .25s", boxShadow: instruction ? `0 0 0 4px ${PURPLE}10` : "none" }}>
                <textarea
                  ...
                  placeholder={chatHistory.length > 0 ? "Refine or add another rule..." : "e.g. Alert me when a worker without a helmet enters the loading zone..."}
                  rows={chatHistory.length > 0 ? 3 : 5}
                  disabled={processing}
                ...
                <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", px: 2.5, py: 1.2, borderTop: `1px solid ${t.border}` }}>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                    ...
                    {chatHistory.length > 0 && (
                      <Box onClick={() => { setChatHistory([]); setInstruction(""); setGeneratedConfig(null); setError(null); try { localStorage.removeItem(CHAT_HISTORY_KEY); } catch {} }}
                        sx={{ cursor: "pointer", px: 1, py: 0.3, borderRadius: "5px", "&:hover": { background: t.surfaceHover } }}>
                        <Typography sx={{ color: t.textMuted, fontSize: ".68rem", opacity: 0.6 }}>Clear chat</Typography>
                      </Box>
                    )}
                  </Box>
                  <Box onClick={canSend ? handleGenerate : undefined}
                    sx={{ display: "flex", alignItems: "center", gap: 1, px: "16px", py: "8px", borderRadius: "10px", background: canSend ? `linear-gradient(135deg, ${PURPLE}, #5B21B6)` : t.surface, border: `1px solid ${canSend ? PURPLE + "70" : t.border}`, cursor: canSend ? "pointer" : "default", boxShadow: canSend ? `0 4px 16px ${PURPLE}40` : "none", transition: "all .2s", "&:hover": canSend ? { transform: "translateY(-1px)" } : {} }}>
                    {processing
                      ? <Box sx={{ width: 13, height: 13, borderRadius: "50%", border: "2px solid rgba(255,255,255,0.2)", borderTopColor: "#fff", animation: "sp 1s linear infinite" }} />
                      : <SendIcon sx={{ fontSize: 14, color: canSend ? "#fff" : t.textMuted }} />}
                    <Typography sx={{ fontSize: ".82rem", fontWeight: 600, color: canSend ? "#fff" : t.textMuted }}>
                ...
                <Box sx={{ p: "8px 12px 12px" }}>
                  {history.length === 0 ? (
                    ...
                    <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
                      {history.map(item => (
                        <Box key={item.id} sx={{ p: "14px 16px", borderRadius: "12px", background: t.surface, border: `1px solid ${t.border}`, transition: "all .25s", "&:hover": { borderColor: `${PURPLE}25` } }}>
