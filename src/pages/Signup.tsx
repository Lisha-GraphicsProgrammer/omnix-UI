...
const inp = {
  '& .MuiOutlinedInput-root': {
    ...
    '&.Mui-focused fieldset': { borderColor: '#2B7A78', borderWidth: 2 },
  ...
  '& .MuiInputLabel-root.Mui-focused': { color: '#3AAFA9' },
    ...
    <Box sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#08080a' }}>
      <Box sx={{ width: '100%', maxWidth: 420, px: 3 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 5, justifyContent: 'center' }}>
          <Box sx={{ width: 36, height: 36, borderRadius: '9px', background: 'linear-gradient(135deg, #2B7A78, #17252A)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 20px rgba(99,102,241,0.4)' }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
              <ellipse cx="12" cy="12" rx="10" ry="6.5" stroke="white" strokeWidth="1.5"/>
              <circle cx="12" cy="12" r="3.5" fill="white"/>
              <circle cx="13.5" cy="10.5" r="1.4" fill="#2B7A78"/>
            </svg>
          </Box>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
          ...
          <Button fullWidth variant="contained" onClick={handleSignup} disabled={loading}
            sx={{
              ...
              background: loading ? 'rgba(99,102,241,0.2)' : 'linear-gradient(135deg, #2B7A78 0%, #17252A 100%)',
              boxShadow: loading ? 'none' : '0 4px 20px rgba(99,102,241,0.28)',
              border: '1px solid rgba(99,102,241,0.3)', transition: 'all .2s ease',
              '&:hover': { background: 'linear-gradient(135deg, #1F5F5D 0%, #123331 100%)', transform: 'translateY(-1px)' },
        ...
        <Box sx={{ mt: 4, textAlign: 'center' }}>
          <Typography sx={{ color: 'rgba(255,255,255,0.3)', fontSize: '.85rem' }}>
            ...
            <Box component="span" onClick={() => nav('/login')}
              sx={{ color: '#3AAFA9', cursor: 'pointer', '&:hover': { color: '#DEF2F1' }, transition: 'color .15s' }}>
              Sign in
            </Box>
          </Typography>
        </Box>
