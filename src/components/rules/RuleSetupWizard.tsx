  ...
  return (
    ...
    >
      ...
      <Box sx={{ px: 3, py: 2.2, borderBottom: `1px solid ${t.border}`, display: "flex", alignItems: "center", gap: 1.5, flexShrink: 0 }}>
        {step === "zone" && (
          <Box onClick={() => setStep("camera")} sx={{ display: "flex", alignItems: "center", cursor: "pointer", color: t.textMuted, "&:hover": { color: t.text } }}>
            <ArrowBackIcon sx={{ fontSize: 18 }} />
          </Box>
        )}
        <Box sx={{ flex: 1 }}>
          <Typography sx={{ color: t.text, fontWeight: 700, fontSize: "1rem" }}>
            {step === "camera" ? "Select a camera" : `Zone on ${camera?.name || "camera"}`}
          </Typography>
